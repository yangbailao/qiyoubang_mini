...
Page({
  ...
  save:function(){
    ...
    if(fromData.phone.length === 0 || fromData.money.length === 0 || fromData.code.length === 0 || fromData.verify_id.length === 0) {
      wx.showToast({
        title: '请输入完内容在提交！',
        icon:'none',
        mask: true
...
if(fromData.money > this.data.max_money){
  wx.showToast({
    title: '提现金额不能大于：'+this.data.max_money,
    icon:'none',
    mask: true
  })
  return
